...
const CartPage = () => {
  ...
  const [discount, setDiscount] = useState(0)
   ...
   useEffect(() => {
      ...
      const discount = cartProduct.reduce((sum, product) => sum + (product.mrp - product.sellingPrice) * product.qty, 0)
                ...
                {cart.products.map((product) => (
                  <tr key={product.variantId} className='md:table-row block border-b'>
                    ...
                    <td className='md:table-cell flex justify-between md:p-3 px-3 pb-2'>
                      ...
                      <div className='flex justify-center'>
                        <div className="flex justify-center items-center md:h-10 h-7 border w-fit rounded-full">
                          <button type="button" className="h-full w-10 flex items-center justify-center cursor-pointer" onClick={() => dispatch(decreaseQuantity({ productId: product.productId, variantId: product.variantId }))}>
                            <HiMinus />
                          </button>
                          <input readOnly type="text" value={product.qty} className="md:w-14 w-8 text-center border-none outline-none" />
                          <button type="button" className="h-full w-10 flex items-center justify-center cursor-pointer" onClick={() => dispatch(increaseQuantity({ productId: product.productId, variantId: product.variantId }))}>
                            <HiPlus />
                          </button>
                        </div>
                      </div>
                    ...
                    <td className='md:table-cell flex justify-between md:p-3 px-3 pb-2 text-center'>
                      ...
                      <button type="button" onClick={() => dispatch(removeFromCart({ productId: product.productId, variantId: product.variantId }))} className='cursor-pointer text-red-500'> <IoCloseCircleOutline /> </button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className='lg:w-[30%] w-full'>
            <div className='rounded bg-gray-50 p-5 sticky top-5'>
              ...
              <div>
                ...
                <Button className="w-full bg-black rounded-full mt-5 mb-3" type="button" asChild>
