...
const ProductDetails = ({ product, variant, colors, sizes, reviewCount }) => {
    ...
    useEffect(() => {
        if (cartStore.count > 0) {
            const exisitingProduct = cartStore.products.findIndex((cartProduct) => cartProduct.productId === product._id && cartProduct.variantId === variant._id)
        ...
        <div className='lg:px-32 px-4'>
            ...
            {isProductLoading &&
                <div className="fixed top-10 left-1/2 -translate-x-1/2 z-50">
                    <Image src={loadingSvg.src} height={80} width={80} alt="Loading" />
                </div>
            }
            ...
            <div className="md:flex justify-between items-start lg:gap-10 gap-5 mb-20">
                ...
                <div className="md:w-1/2 md:mt-0 mt-5">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                        ...
                        <span className="bg-red-500 rounded-2xl px-3 p-1 text-white text-xs ms-5">-{variant?.discountPercentage}%</span>
                    </div>
                        ...
                        <div className="flex gap-5">
                            {colors.map(color => (
                                <Link
                                    ...
                                    key={color} href={`${WEBSITE_PRODUCT_DETAILS(product.slug)}?color=${color}&size=${variant.size}`}>{color}</Link>
                            ))}
                        </div>
                    </div>
                        ...
                        <div className="flex gap-5">
                            {sizes.map(size => (
                                <Link
                                    ...
                                    key={size} href={`${WEBSITE_PRODUCT_DETAILS(product.slug)}?color=${variant.color}&size=${size}`}>{size}</Link>
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="mt-5">
                        ...
                        <div className="flex items-center h-10 border w-fit rounded-full">
                            <button type="button" className="h-full w-10 flex items-center justify-center cursor-pointer" onClick={() => handleQty('desc')}>
                                <HiMinus />
                            </button>
                            <input readOnly type="text" value={qty} className="w-14 text-center border-none outline-none" />
                            <button type="button" className="h-full w-10 flex items-center justify-center cursor-pointer" onClick={() => handleQty('inc')}>
                                <HiPlus />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="mt-5">
                        {!isAddedIntoCart ?
                            <ButtonLoading onClick={handleAddToCart} type="button" text='Add To Cart' className="w-full rounded-full py-6 text-md" />
                            :
