      ...
      <Card className='py-0 rounded shadow-sm'>
        ...
        <CardContent className="pb-5">
          ...
          {selectedMedia.length > 0 &&
            <div className='py-2 px-3 bg-violet-200 mb-2 rounded-2xl flex justify-between items-center '>
              <Label>
                <Checkbox
                  checked={selectAll}
              ...
              <div className='flex gap-2'>
                {deleteType === "SD" ?
                  <Button variant='destructive' onClick={() => handleDelete(selectedMedia, deleteType)}>Move into Trash</Button> :
                  <>
                    <Button className="bg-green-500 hover:bg-green-600" onClick={() => handleDelete(selectedMedia, "RSD")}>Restore</Button> 
                    <Button variant='destructive' onClick={() => handleDelete(selectedMedia, deleteType)}>Delete Permanently</Button> 
          ...
          {status === 'pending' ? <div>Loading...</div> :
            status === 'error' ? <div className='text-red-500 text-sm'>{error.message}</div> :
            <>
            {data.pages.flatMap(page => page.mediaData.map(media => media._id)).length === 0 && <div>No media found.</div>}
